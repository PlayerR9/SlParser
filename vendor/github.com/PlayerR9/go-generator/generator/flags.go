...
func (s StructFieldsVal) String() string {
	...
	fn := func(elem any) error {
		entry := dbg.AssertConv[entry[string, string]](elem, "elem")
	...
	err := itr.Iterate(s.fields.Iterator(), fn)
	dbg.AssertErr(err, "iterator.Iterate(s.fields.Iterator(), fn) failed")
...
func (s *StructFieldsVal) MakeParameterList() (string, error) {
	...
	fn := func(elem any) error {
		...
		dbg.AssertOk(ok, "slices.BinarySearch(field_list, %q)", entry.Key)
