...
func init() {
	Logger = ggen.InitLogger(os.Stdout, "SlParser")
}

func main() {
	ggen.ParseFlags()

	g, err := ggen.NewCodeGeneratorFromTemplate[*internal.GenData]("node", templ)
	if err != nil {
		Logger.Fatalf("Failed to create code generator: %v", err)
	}

	gd := &internal.GenData{}

	res, err := g.Generate(internal.OutputLocFlag, "node.go", gd)
	if err != nil {
	...
	Logger.Printf("Successfully generated %q\n", res.DestLoc)
}

const templ string = `// Code generated by go generate; DO NOT EDIT
package {{ .PackageName }}
